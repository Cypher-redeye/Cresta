...
def signup(request):
    ...
    try:
        ...
        verification_url = f"{settings.FRONTEND_URL}/verify-email?token={token}&email={encoded_email}"
...
def verify_email(request):
    ...
    try:
        ...
        if profile.email_verified and not profile.verification_token_hash:
            tokens = get_tokens_for_user(user)
            return Response({
...
def forgot_password(request):
    ...
    return Response({'message': 'If an account exists, a reset email has been sent.'}, status=status.HTTP_200_OK)
